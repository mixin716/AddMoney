	...
	public static List<BuyProductsBean> parseBuyProducts(String json){
		...
		try {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				bean.setmWf(fundinfoObj.optString("hf_incomeratio"));//万分收益
				bean.setmHave(obj.optString("currentremainshare"));//持有金额
				bean.setmName(obj.optString("freezeremainshare"));//未结算收益
				float aaa,bbb,ccc;
				aaa = Float.valueOf(fundinfoObj.optString("hf_incomeratio"));
